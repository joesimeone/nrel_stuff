...
nrel_tbl <-
  ...
  filter(
    out_load_cooling_energy_delivered_kbtu > 500, 
    out_load_heating_energy_delivered_kbtu > 500, ## People not really using their system
  ...
  mutate(out_total_temp_load = out_load_cooling_energy_delivered_kbtu + out_load_heating_energy_delivered_kbtu) |> 
  select(
    bldg_id,
    #  weight,
    in_county,
...
dbExecute(
  con,
  "CREATE SCHEMA IF NOT EXISTS total_train;"
)


dbExecute(
  con,
  "CREATE SCHEMA IF NOT EXISTS total_test;"
)
...
map2(
  ...
  ~ DBI::dbWriteTable(
    con,
    DBI::Id(schema = "total_train", table = str_glue('{.y}')),
    value = .x,
    overwrite = TRUE
  )
)
...
map2(
  ...
  ~ DBI::dbWriteTable(
    con,
    DBI::Id(schema = "total_test", table = str_glue('{.y}')),
    value = .x,
    overwrite = TRUE
  )
)
...
nrel_pre_spec <-
  map(nrel_train, function(dat) {
    recipe(out_total_temp_load ~ ., data = dat) |>
      update_role(
        bldg_id,
        in_county,
        in_puma,
...
tot_model_outputs <- pmap(
  list(
    dat = tot_wflow_set,
    folds = nrel_folds,
    name = names(tot_wflow_set)
  ),
  in_parallel(
...
tot_metrics <-
  map(tot_model_outputs, collect_metrics)

tot_metrics_fin <-
  imap(tot_metrics, ~ .x |> mutate(clim_reg = .y)) |>
  list_rbind()


tot_pred <-
  map(tot_model_outputs, collect_predictions)
...
dbExecute(
  con,
  "CREATE SCHEMA IF NOT EXISTS tot_model_comps;"
)


DBI::dbWriteTable(
  con,
  DBI::Id(schema = "tot_model_comps", table = "ashrae_climate_region_metrics"),
  tot_metrics_fin
)
