...
dbExecute(
  con,
  "CREATE SCHEMA IF NOT EXISTS heating_train;"
)


dbExecute(
  con,
  "CREATE SCHEMA IF NOT EXISTS heating_test;"
)
...
map2(
  ...
  ~ DBI::dbWriteTable(
    con,
    DBI::Id(schema = "heating_train", table = str_glue('{.y}')),
    value = .x,
    overwrite = TRUE
  )
)
...
map2(
  ...
  ~ DBI::dbWriteTable(
    con,
    DBI::Id(schema = "heating_test", table = str_glue('{.y}')),
    value = .x,
    overwrite = TRUE
  )
)
...
nrel_pre_spec <-
  map(nrel_train, function(dat) {
    recipe(out_load_heating_energy_delivered_kbtu ~ ., data = dat) |>
      update_role(
        bldg_id,
        in_county,
        in_puma,
...
heat_model_outputs <- pmap(
  list(
    dat = heat_wflow_set,
    folds = nrel_folds,
    name = names(heat_wflow_set)
  ),
  in_parallel(
  ...
  set_names(names(heat_wflow_set))
...
heat_metrics <-
  map(heat_model_outputs, collect_metrics)

heat_metrics_fin <-
  imap(heat_metrics, ~ .x |> mutate(clim_reg = .y)) |>
  list_rbind()


heat_pred <-
  map(heat_model_outputs, collect_predictions)
...
dbExecute(
  con,
  "CREATE SCHEMA IF NOT EXISTS heat_model_comps;"
)


DBI::dbWriteTable(
  con,
  DBI::Id(schema = "heat_model_comps", table = "ashrae_climate_region_metrics"),
  heat_metrics_fin
)
